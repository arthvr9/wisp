...
describe('parseSheet', () => {
  it('groups frames per pose tag', () => {
    ...
    expect(sheet.animations.idle).toEqual([
      { x: 0, y: 0, w: 32, h: 32, durationMs: 500 },
      { x: 32, y: 0, w: 32, h: 32, durationMs: 500 },
    ]);
  ...
  it('throws when a pose tag is missing', () => {
    const broken: AsepriteJson = {
      ...json,
      meta: { frameTags: json.meta.frameTags.filter((t) => t.name !== 'sleep') },
    };
  ...
  it('throws when a tag points past the last frame', () => {
    const broken: AsepriteJson = {
      ...json,
      meta: {
        frameTags: json.meta.frameTags.map((t) => (t.name === 'drag' ? { ...t, to: 9 } : t)),
      },
    };
    expect(() => parseSheet(broken)).toThrow('sheet has 7 frames');
  });
});
...
describe('frameAt', () => {
  const frames: Frame[] = [
    ...
    { x: 32, y: 0, w: 32, h: 32, durationMs: 200 },
    { x: 64, y: 0, w: 32, h: 32, durationMs: 50 },
  ];
